...
baseline_file = st.file_uploader("Upload OL Complete file", type=["csv", "xlsx"])
data_file = st.file_uploader("Upload Corn data file", type=["csv", "xlsx"])

if baseline_file and data_file:
    # ---------------------------
    # Read files
    # ---------------------------
    if baseline_file.name.lower().endswith(".csv"):
